...
module.exports = {
  ...
  plugins: [
    plugin(function({ addBase, config }) {
      addBase({
        a: { color: config("theme.textColor.anchor") },
        "a:hover": { color: config("theme.textColor.anchor-hover") },
        body: { lineHeight: "1.5", letterSpacing: "0" },
        "button:focus": { outline: "none" },
        h1: { fontSize: config("theme.fontSize.2xl") },
        h2: { fontSize: config("theme.fontSize.xl") },
        h3: { fontSize: config("theme.fontSize.lg") },
        p: { marginBottom: config("theme.spacing.4") },
        code: { fontFamily: config("theme.fontFamily.mono") }
      });
    }),
    plugin(function({ addComponents, theme }) {
      ...
      addComponents([card, button, list, logo, menu, navbar, template]);
    })
  ]
};
